...
for pid_ in pid:
    ps_call = subprocess.run('sudo docker ps --no-trunc | grep $(cat /proc/'+str(pid_)+'/cgroup | grep -oE "[0-9a-f]{64}" | head -1) | sed "s/^.* //"', shell=True, stdout=subprocess.PIPE)
    name = ps_call.stdout.decode()
    container_name.append(name.split('_')[0])
